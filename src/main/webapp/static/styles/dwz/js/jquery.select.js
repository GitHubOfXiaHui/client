...
(function($) {
					...
					$.RelateSelect,
					{
							...
							createSelect : function(deepth) {
								...
								if (deepth > 0) {
									for ( var i = 0; i < deepth; i++) {
										var $option = this.selectors[i]
												.children(":selected"), key = $option
												.text()
												+ ',' + $option.val();
										data = data[key];
									}
								...
								if (isNew) {
									$select.append(
											$.RelateSelect
													.createOptionsHtml(data))
											.val(s.selecteds[deepth] || -1000)
											.appendTo(this.container).trigger(
													"change");
								} else {
									$select.empty().append(
											$.RelateSelect
						...
						createOptionsHtml : function(data) {
							...
							for ( var i = 0; data[i]; i++) {
								...
								options.push([ "<option value=\"", tmp[1],
										"\">", tmp[0], "</option>" ].join(""));
							}
	...
	$.fn.relateSelect = function(options) {
		...
		if (this.length > 0) {
			$.data(this[0], 'RelateSelect',
					new $.RelateSelect(this[0], options));
		}
	};
